...
import { triangle, ellipse, square, arrowBack, arrowForward, personCircleOutline, globeOutline, removeCircleSharp, removeOutline, addOutline, informationCircle, star, chevronBack, heart, heartOutline } from 'ionicons/icons';
...
import { decdateact, setcategories, setlike_list, deletelike_list } from '../../Feature/DeclencheursSlice';
...
export const Conteneur: React.FC<Ajout_utiliformprops> = ({ Nom, Prix, Id, Stock, Ig, }) => {
    ...
    let article: any = ([].concat(useSelector((state: any) => state.product.product)))
    const [totalsold, setTotalsold] = useState((useSelector((state: any) => state.product.product)).filter((t:any) => t.id == Id)[0].total_sold);
    ...
    // };
    // downloadingImage.src =`https://backend-shop.benindigital.com/${Ig}`
    ...
    const majlike = (n: any | React.SetStateAction<any>) => {
        Axios.put('https://backend-shop.benindigital.com/majlike', {
            id: Id,
    ...
    // }, 10000);
    const declike = () => {
        fetch('https://backend-shop.benindigital.com/obtimg').then((res) => {
            const data = res
            return data
        }).then((data) => {
            console.log(data);
    ...
    const convertimg = async () => {
        // console.log("rr");
        var myImage: any = document.querySelector('.imga');
        fetch(`https://backend-shop.benindigital.com/${Ig}`)
            .then((res) => {
                console.log(res);
            })
    }
    ...
    return (
        <>
            ...
            {likeList.includes(Id) ? (
                ...
                <IonIcon icon={heart} className="favbadge" color="danger" size="large" onClick={() => { dispatch(deletelike_list(Id)); majlike(0) }} />
            ) : (
                <IonIcon icon={heartOutline} className='favbadge' color="danger" size="large" onClick={() => { dispatch(setlike_list(Id)); majlike(1) }} />
            )
            }
                        ...
                        <IonRow className="r3">
                            ...
                            <IonCol className="c2" >
                                <div onClick={() => {
                                    ...
                                    //  setShowLoading(true);
                                }} className='addn' ><AddToCartButton icon={true} /></div>
                            </IonCol>
                        </IonRow>
                    ) : (
